...
#define __ELRS_H__
#include "main.h"
#define MAX_FRAME_SIZE 64 // 空闲中断接收的最大帧长
extern uint8_t elrs_data_temp[64];
/*
...
typedef struct
{
    ...
    float Left_X;          // 左摇杆x轴ch4
    float Left_Y;          // 左摇杆y轴ch3
    float Right_X;         // 右摇杆x轴ch1
    float Right_Y;         // 右摇杆y轴ch2
    //float S1;              // 左滑块
    //float S2;              // 右滑块
    uint8_t A;             // 按键Ach5
    uint8_t B;             // 三档按键Bch6
