...
void MX_USART10_UART_Init(void)
{
  ...
  // 启用 FIFO 模式
  HAL_UARTEx_EnableFifoMode(&huart10);
  // 设置接收 FIFO 阈值为 1/2（即接收到一半触发 DMA 请求）
  HAL_UARTEx_SetRxFifoThreshold(&huart10, UART_RXFIFO_THRESHOLD_1_2);
  // 发送 FIFO 阈值可默认
  HAL_UARTEx_SetTxFifoThreshold(&huart10, UART_TXFIFO_THRESHOLD_1_8);
