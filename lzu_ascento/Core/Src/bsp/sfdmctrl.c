...
/**
************************************************************************
* @brief:      	3510&stackforce_fbdata: 获取电机反馈数据函数
...
void dm6215_fbdata(Wheel_Motor_t *motor, uint8_t *rx_data,uint32_t data_len)
{
	...
	{//返回的数据有8个字节
	  ...
	  motor->para.vel = uint_to_float(motor->para.v_int, V_MIN, V_MAX, 12); // (-280.0,280.0)
	  motor->para.tor = uint_to_float(motor->para.t_int, T_MIN, T_MAX, 12);  // (-1.0,1.0)
	  motor->para.Tmos = (float)(rx_data[6]);
	  motor->para.Tcoil = (float)(rx_data[7]);
	}
}

void sf_fbdata(Joint_Motor_t *motor, uint8_t *rx_data,uint32_t data_len)
{
	if(data_len==FDCAN_DLC_BYTES_8)
	{//返回的数据有8个字节
		if(rx_data[0] != 0x01) {
		...
		motor->para.p_int = (rx_data[1] << 8) | rx_data[2];
		motor->para.pos = uint_to_float(motor->para.p_int, -25.12f, 25.12f, 16)/8.0f;
		motor->para.v_int = (rx_data[3] << 4) | (rx_data[4] >> 4);
		motor->para.vel = uint_to_float(motor->para.v_int, -65.0f, 65.0f, 12);
		motor->para.t_int = ((rx_data[4] & 0x0F) << 8) | rx_data[5];
		motor->para.tor = uint_to_float(motor->para.t_int, -18.0f, 18.0f, 12);
	}
}
/**
************************************************************************
* @brief:      	disable_motor_mode: 使能电机模式函数
* @param[in]:   hcan:     指向CAN_HandleTypeDef结构的指针
* @param[in]:   motor_id: 电机ID，指定目标电机
* @param[in]:   mode_id:  模式ID，指定要使能的模式
* @retval:     	void
* @details:    	通过CAN总线向特定电机发送使能特定模式的命令
************************************************************************
**/
...
/**
************************************************************************
* @brief:      	disable_motor_mode: 设置电机零位
* @param[in]:   hcan:     指向CAN_HandleTypeDef结构的指针
* @param[in]:   motor_id: 电机ID，指定目标电机
* @param[in]:   mode_id:  模式ID
* @retval:     	void
* @details:    	通过CAN总线向特定电机发送设置零位的命令
************************************************************************
**/
...
************************************************************************
**/
void mit_ctrl1(hcan_t* hcan, uint16_t motor_id, float pos, float vel,float kp, float kd, float torq)
{
	uint8_t data[8];
