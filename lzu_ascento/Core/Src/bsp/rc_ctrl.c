...
void rcdata_ctrl(void)
{
	...
	if (cur_E == 1 && last_E != 1 && chassis_move.prejump_flag == 1 && chassis_move.jump_flag==0) {
	...
	if(chassis_move.start_flag==1) {
		...
		mySaturate(&chassis_move.roll_set,-0.8f,0.8f);
		mySaturate(&chassis_move.leg_set,0.10f,0.28f);
		...
		if(chassis_move.movejump_flag==1)
		{
			...
			chassis_move.x_set=chassis_move.x_set+chassis_move.v_set*0.004f;
			movetime++;
			if(movetime>200)
			{
				movetime=0;
