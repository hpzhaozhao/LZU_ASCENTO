...
uint8_t elrs_data_temp[64] __attribute__((section(".dma_buffer")));

void ELRS_Init(void)
{
    HAL_StatusTypeDef status;
    status = HAL_UARTEx_ReceiveToIdle_DMA(&huart10, elrs_data_temp, MAX_FRAME_SIZE);
    if (status != HAL_OK) {
        printf("ELRS_Init DMA start failed: %d\r\n", status);
    } else {
...
void ELRS_UARTE_RxCallback(uint16_t Size)
{
    ...
    if (elrs_data_temp[0] == CRSF_ADDRESS_FLIGHT_CONTROLLER)
    {
        ...
        else if (elrs_data_temp[2] == CRSF_FRAMETYPE_RC_CHANNELS_PACKED) // 数据帧类型为RC通道数据
        {
            ...
            elrs_data.A = elrs_data.channels[4] == 992 ? 1 : (elrs_data.channels[4] == 1792 ? 2 : 0);
            ...
            elrs_data.D = elrs_data.channels[7] == 992 ? 1 : (elrs_data.channels[7] == 1792 ? 2 : 0);
            elrs_data.E= elrs_data.channels[8] > 1000 ? 1 : 0;
    ...
    // printf("RX Size=%d: ", Size);
    // for (int i = 0; i < Size && i < 16; i++) {
    //     printf("%02X ", elrs_data_temp[i]);
    // }
    // printf("\r\n");
